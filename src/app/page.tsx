...
const steps = [
  { num: '01', title: 'Conta o que você fez', desc: 'Não fique olhando para uma página em branco. Só fala para a IA sua experiência com suas próprias palavras — sem formalidade, sem pressão.', tag: 'IA PROCESSANDO', preview: '"Trabalhei 3 anos como dev React, fiz projetos de e-commerce..."' },
  { num: '02', title: 'A IA formata tudo', desc: 'Nossa IA transforma suas palavras em bullet points profissionais, organiza seções e otimiza o texto para passar em sistemas ATS automaticamente.', tag: 'FORMATANDO', preview: '• Desenvolveu 8 projetos de e-commerce com React, aumentando conversão em 40%' },
  { num: '03', title: 'Baixa e aplica', desc: 'Exporte em PDF com um clique. Seu currículo está otimizado para ATS, com design profissional e pronto para conquistar a vaga dos seus sonhos.', tag: 'PDF PRONTO', preview: '✓ Currículo gerado · ✓ ATS otimizado · ✓ Pronto para enviar' },
]

const faqs = [
  { q: 'O Vaga.ai é gratuito?', a: 'Sim! O plano gratuito inclui geração de currículo com IA e acesso ao chat de carreira. O plano Pro desbloqueia currículos ilimitados, cartas de apresentação e preparação para entrevistas.' },
  { q: 'O currículo gerado passa em sistemas ATS?', a: 'Sim! Nossa IA foi treinada para criar currículos otimizados para sistemas ATS (Applicant Tracking Systems), aumentando suas chances de chegar até a entrevista.' },
  { q: 'Preciso saber escrever bem para usar?', a: 'Não! Você só precisa descrever sua experiência com suas palavras — pode ser informal, incompleto, do jeito que vier. A IA transforma em linguagem profissional.' },
  { q: 'Em quanto tempo o currículo fica pronto?', a: 'Em menos de 45 segundos. Você preenche o formulário, clica em gerar e o currículo profissional aparece na tela pronto para baixar.' },
  { q: 'Posso gerar vários currículos diferentes?', a: 'No plano gratuito você pode gerar 1 currículo. No plano Pro você gera currículos ilimitados, personalizados para cada vaga que quiser.' },
]
    ...
    <main style={{ minHeight: '100vh', background: '#0A0A0F', color: '#F0EEE8', fontFamily: "'Space Grotesk', system-ui, sans-serif", overflowX: 'hidden' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 3rem', borderBottom: '1px solid #1E1E2E', position: 'sticky', top: 0, zIndex: 100, background: 'rgba(10,10,15,0.85)', backdropFilter: 'blur(20px)' }}>
        ...
        <div style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
          ...
          <a href="/curriculo" style={{ background: '#C8FA5F', color: '#0A0A0F', padding: '0.55rem 1.4rem', borderRadius: '100px', fontWeight: 700, fontSize: '0.875rem', textDecoration: 'none', transition: 'transform 0.2s, box-shadow 0.2s', boxShadow: '0 0 20px rgba(200,250,95,0.2)' }}>Criar currículo →</a>
        </div>
      </nav>

      {/* HERO */}
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '7rem 3rem 5rem', position: 'relative' }}>
        <div style={{ position: 'absolute', top: '10%', right: '5%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(200,250,95,0.06) 0%, transparent 70%)', pointerEvents: 'none' }} />
        
        <div ref={heroRef} style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(30px)', transition: 'opacity 0.8s ease, transform 0.8s ease' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(200,250,95,0.08)', border: '1px solid rgba(200,250,95,0.2)', color: '#C8FA5F', padding: '0.35rem 1rem', borderRadius: '100px', fontSize: '0.8rem', fontWeight: 600, marginBottom: '2rem', letterSpacing: '0.05em' }}>
            <span style={{ width: 6, height: 6, background: '#C8FA5F', borderRadius: '50%', display: 'inline-block', animation: 'pulse-ring 1.5s infinite' }} />
          ...
          <p style={{ fontSize: '1.15rem', color: '#7A7A8A', maxWidth: '520px', marginBottom: '2.5rem', lineHeight: '1.7' }}>
            Descreve sua experiência do jeito que vier. Nossa IA transforma em currículo profissional, otimizado para ATS, pronto para baixar.
          </p>

          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '4rem' }}>
            <a href="/curriculo" style={{ background: '#C8FA5F', color: '#0A0A0F', padding: '0.9rem 2rem', borderRadius: '100px', fontWeight: 700, fontSize: '1rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', boxShadow: '0 0 40px rgba(200,250,95,0.25)', animation: 'float 3s ease-in-out infinite' }}>
              Criar currículo grátis ✨
            </a>
            <a href="/chat" style={{ color: '#7A7A8A', padding: '0.9rem 2rem', borderRadius: '100px', fontWeight: 600, fontSize: '1rem', textDecoration: 'none', border: '1px solid #1E1E2E', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              Conversar com IA →
            </a>
          </div>

          <div style={{ display: 'flex', gap: '3rem', paddingTop: '3rem', borderTop: '1px solid #1E1E2E', flexWrap: 'wrap' }}>
            {[['10k+', 'Currículos gerados'], ['98%', 'Taxa de satisfação'], ['45s', 'Tempo médio'], ['3×', 'Mais entrevistas']].map(([num, label]) => (
              <div key={label}>
      ...
      <div style={{ overflow: 'hidden', borderTop: '1px solid #1E1E2E', borderBottom: '1px solid #1E1E2E', padding: '1rem 0', background: '#13131A' }}>
        <div style={{ display: 'flex', gap: '4rem', animation: 'marquee 20s linear infinite', whiteSpace: 'nowrap' }}>
          {['Currículo com IA', 'ATS Otimizado', 'Chat de Carreira', 'PDF em 1 clique', 'Sem experiência necessária', 'Currículo com IA', 'ATS Otimizado', 'Chat de Carreira', 'PDF em 1 clique', 'Sem experiência necessária'].map((item, i) => (
            <span key={i} style={{ color: '#7A7A8A', fontSize: '0.85rem', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: '0.75rem' }}>
              <span style={{ width: 4, height: 4, background: '#C8FA5F', borderRadius: '50%', display: 'inline-block', flexShrink: 0 }} />
              {item}
            </span>
          ))}
        </div>
      ...
      <section id="como-funciona" style={{ padding: '7rem 3rem', maxWidth: '1100px', margin: '0 auto' }}>
        <AnimatedSection>
          <div style={{ textAlign: 'center', marginBottom: '5rem' }}>
            ...
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(2rem, 4vw, 3.5rem)', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.1 }}>Do zero ao currículo.<br /><span style={{ color: '#C8FA5F' }}>Em 3 passos.</span></h2>
          </div>
        </AnimatedSection>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', background: '#1E1E2E', borderRadius: '20px', overflow: 'hidden', border: '1px solid #1E1E2E' }}>
          {steps.map((step, i) => (
            <AnimatedSection key={i} delay={i * 0.15}>
              <div style={{ background: activeStep === i ? '#16161F' : '#13131A', padding: '2.5rem', cursor: 'pointer', transition: 'background 0.3s', borderLeft: activeStep === i ? '3px solid #C8FA5F' : '3px solid transparent' }}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {activeStep === i && (
                      <div style={{ background: '#0A0A0F', border: '1px solid #1E1E2E', borderRadius: '12px', padding: '1rem 1.25rem', fontFamily: 'monospace', fontSize: '0.875rem', color: '#C8FA5F', lineHeight: '1.6' }}>
                        ...
                        {step.preview}
                        {i === 0 && <span style={{ animation: 'blink 1s infinite' }}>|</span>}
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <AnimatedSection>
            <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
              ...
              <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 800, letterSpacing: '-0.03em' }}>Tudo que você precisa.<br />Num só lugar.</h2>
            </div>
          </AnimatedSection>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            {[
              { icon: '🤖', title: 'Vaga AI', desc: 'Chat inteligente que entende seu perfil, analisa sua experiência e dá conselhos personalizados de carreira 24/7.', href: '/chat' },
              { icon: '📄', title: 'Currículo com IA', desc: 'Descreve com suas palavras. A IA formata, otimiza para ATS e gera um PDF profissional em menos de 1 minuto.', href: '/curriculo' },
              { icon: '✍️', title: 'Carta de Apresentação', desc: 'Personalizada para cada vaga. A IA analisa a descrição da vaga e cria uma carta irresistível para o recrutador.', href: '/curriculo' },
              { icon: '🎯', title: 'Prep de Entrevistas', desc: 'Simule entrevistas com a IA. Receba perguntas reais da vaga e feedback detalhado nas suas respostas.', href: '/chat' },
              { icon: '🌍', title: 'Tradução de Documentos', desc: 'Traduza seu currículo para inglês, espanhol ou qualquer idioma mantendo o formato profissional intacto.', href: '/chat' },
              { icon: '⚡', title: 'Otimização ATS', desc: 'Nossa IA analisa a vaga e ajusta seu currículo para maximizar sua pontuação nos sistemas de triagem automática.', href: '/curriculo' },
            ].map((f, i) => (
              <AnimatedSection key={i} delay={i * 0.1}>
                <a href={f.href} style={{ background: '#0A0A0F', border: '1px solid #1E1E2E', borderRadius: '16px', padding: '2rem', textDecoration: 'none', display: 'block', height: '100%', transition: 'border-color 0.2s, transform 0.2s' }}
                  ...
                  <h3 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: '1.1rem', marginBottom: '0.5rem', color: '#F0EEE8' }}>{f.title}</h3>
            ...
            <AnimatedSection key={i} delay={i * 0.05}>
              <div style={{ background: '#13131A' }}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  style={{ width: '100%', padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', color: '#F0EEE8', fontSize: '1rem', fontWeight: 600, cursor: 'pointer', textAlign: 'left', fontFamily: "'Space Grotesk', sans-serif" }}>
                  {faq.q}
      ...
      <section style={{ padding: '7rem 3rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <AnimatedSection>
          ...
          <p style={{ color: '#7A7A8A', fontSize: '1.1rem', marginBottom: '2.5rem' }}>Junte-se a milhares de profissionais que já usam IA para se destacar.</p>
      ...
      <footer style={{ borderTop: '1px solid #1E1E2E', padding: '2rem 3rem' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <p style={{ color: '#7A7A8A', fontSize: '0.8rem' }}>© 2026 Vaga.ai — Todos os direitos reservados</p>
          <div style={{ display: 'flex', gap: '1.5rem' }}>
            <a href="#" style={{ color: '#7A7A8A', textDecoration: 'none', fontSize: '0.8rem' }}>Privacidade</a>
            <a href="#" style={{ color: '#7A7A8A', textDecoration: 'none', fontSize: '0.8rem' }}>Termos</a>
